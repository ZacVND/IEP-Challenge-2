...
//  Serial.print("\r\n"); 
  ...
  void PHcommandListen()  // listen and reactor to commands from the interface
  {
       ...
       while (Serial.available() > 0) {  // if theres's something in the serial buffer
          
         String inString = Serial.readString();  // read a string from the serial buffer
         inString.trim();  // remove any blank spaces in the string
         //Serial.println(inString);  //DEBUG ONLY
         
         if(inString.equals("S")){  // if we see shutdown command
         ...
         if(inString.equals("XS")){  // if we see resume command
         //Serial.println("PH has resumed");
         shutDown = false;
         }
         
         if((inString.substring(0,11)).equals("PHcommanded"))  // if we see a command and it begines with PHcommanded
         {
           PHtarget = inString.substring(12).toInt();  // change the target PH to the last values of the command
         }
         }
